...
load(file = "results/consensus_gene_lists.RData")
```

Load in file containing chromosome information:

```{r, message = FALSE}
## Read in gene coordinates for each gene:
...
Add a column containing status:

```{r, message = FALSE}
## Reformat gene names for subsetting:
phen_not_env_genes <- gsub(pattern = "[.]1",
                           replacement = "",
                           phen_not_env_genes)
stress_genes <- gsub(pattern = "[.]1",
                           replacement = "",
                           stress_genes)
osm_env_response_genes <- gsub(pattern = "[.]1",
                           replacement = "",
                           osm_env_response_genes)

## Filter based on chromosome:
combined_filtered <- subset(gene_location_info,
                            as.numeric(chrom) <= 40)

smolt_vs_non_smolts_fresh_df$locus <- gsub(pattern = "[.]1",
                                           replacement = "",
                                           row.names(smolt_vs_non_smolts_fresh_df))

combined_filtered$locus <- as.character(unlist(combined_filtered$locus))

combined_filtered <- combined_filtered[match(combined_filtered$locus,
                             smolt_vs_non_smolts_fresh_df$locus),]

smolt_vs_non_smolts_new <- smolt_vs_non_smolts_fresh_df[match(combined_filtered$locus,
                                                              smolt_vs_non_smolts_fresh_df$locus),]

combined_filtered$log2fc <- smolt_vs_non_smolts_new$log2FoldChange

## Combine all into one vector:
combined_genes_of_interest <- c(phen_not_env_genes,
                                stress_genes,
                                osm_env_response_genes)

combined_filtered$status <- 0
combined_filtered[combined_filtered$locus %in% combined_genes_of_interest, ]$status <- 1


alh <- subset(combined_filtered,
       group == "ALH")

## Subset genes only found on full chromosomes:
combined_filtered$group <- "Other"
combined_filtered[combined_filtered$locus %in% phen_not_env_genes, ]$group <- "ALH"
combined_filtered[combined_filtered$locus %in% stress_genes, ]$group <- "Stress"
combined_filtered[combined_filtered$locus %in% osm_env_response_genes, ]$group <- "OSM"
...
don <- combined_sorted %>%
  ...
  mutate(is_highlight = ifelse(locus %in% combined_genes_of_interest,
                                "yes",
                                "no"))
...
## Save image:
ggsave(file = "results/manhattan.png",
       height = 8,
       width = 14)
```

Try chrom-level scatterplots:

```{r, message = FALSE}
ggplot(data = combined_sorted,
       aes(x = start,
           y = log2fc)) +
        geom_point() +
       geom_point(data = subset(combined_sorted,
                             group == "ALH"),
               color = "green",
               alpha = 0.8,
               size = 1) +
